...
| `/api/v1/test-connection` | `POST` | Bearer-authenticated smoke test — wraps `processChatCompletion` with a locked prompt (`"Reply with the single word 'ok' and nothing else."`, `max_tokens=5`, `temperature=0`) against the cheapest active model in the caller's allowlist. Success envelope: `success: true`, `user_type`, `model_used`, `response_text`, `cost_usd_cents`, `cost` (`usd_cents` + `display`), `budget` (`remaining_usd_cents`, `total_usd_cents`, `display`), `latency_ms`. Failure envelope: `success: false`, `user_type`, `error: { code, message, hint }` where `code` is one of `no_providers_active`, `no_models_in_tier`, `budget_exhausted`, `rate_limited`, `provider_error`, `unknown`, and `hint` branches by `user_type` (`team_admin` / `team_member` / `voucher_recipient`). |
| `/api/v1/test-connection/session` | `POST` | Same envelope as the bearer endpoint, but cookie-authenticated via the dashboard session. Lets logged-in users (including OAuth-only members with no pasteable key) exercise the same sanity check from the "Test your key" button. The membership must belong to the calling user — admins cannot test on behalf of other members from this endpoint. Auto-invoked by the redemption flow so vouchers are validated end-to-end before the recipient ever leaves the redeem page. |

`/api/v1/chat/completions`, `/api/v1/models`, and `/api/v1/test-connection` require `Authorization: Bearer allotly_sk_...` headers; `/api/v1/test-connection/session` is cookie-authenticated. The two test-connection endpoints share a single shared core (`runTestConnection` in `server/lib/proxy/test-connection.ts`) that reuses the full proxy pipeline — budget reservation, model allowlist checks, rate-limit + concurrency, Redis cost reconciliation. Short-circuit paths (no_providers_active, no_models_in_tier, pre-call budget_exhausted) still consume a rate-limit slot so this endpoint cannot be used to probe org state. The internal classifier (`classifyError`) maps every internal safeguard code onto exactly one of the six user-facing codes — the original safeguard code never leaks. Provider names and upstream payloads are also stripped from the user-facing `message` and `hint` fields.
